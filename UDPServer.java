		   ...
		   // public static ArrayList <Client> network = new ArrayList <Client>();
			while(true) 
			{
				...
				if (network.size() == 0)
				{
					...
					String r1 = "add-" + newClient.getIP().getHostAddress();
					byte[] s1 = r1.getBytes();
				...
				for (int i = 0; i<network.size(); i++)
				{
					...
					if (j == 1)
					{
						...
						String r1 = "rmv-" + network.get(i).getIP().getHostAddress();
						byte[] s1 = r1.getBytes();
						DatagramPacket sp = new DatagramPacket(s1, s1.length, address, port);
						for (int k=0; k<network.size(); k++)
						{
						...
						/*After each computer is removed, send the updated network arraylist to all the clients
						...
						for (int k=0; k<network.size(); k++)
						{
							DatagramPacket sendPacketofNetwork = new DatagramPacket(data, data.length, network.get(k).getIP(), 2096);
							socket2.send(sendPacketofNetwork);
						}
						*/
					}
